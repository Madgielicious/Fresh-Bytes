...
function renderServiceCards() {
  ...
  SERVICES_DATA.forEach((service) => {
    ...
    card.className = 'service-card reveal cursor-pointer';
    card.innerHTML = `
...
function openServiceModal(service) {
  ...
  modalBody.innerHTML = `
    <div class="modal-service-header">
      <span class="modal-service-tag">${service.tag}</span>
      <h2>${service.name}</h2>
    </div>
    ...
    <div class="modal-service-content">
      ...
      <section class="modal-section office-section">
        <h3>Office Information</h3>
        <div class="office-info">
          <p><strong>${service.office.label}</strong></p>
          <p><strong>Hours:</strong> ${service.office.hours}</p>
          <p><strong>Note:</strong> ${service.office.note}</p>
          <p>
            <a href="${service.office.link}" target="_blank" class="service-link">
              ${service.office.linkLabel} ↗
            </a>
          </p>
        </div>
      </section>
