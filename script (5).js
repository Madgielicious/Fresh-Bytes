// Hero search redirects to the services page, with live suggestions
const heroSearchForm = document.getElementById('heroSearchForm');
const heroSearchInput = document.getElementById('heroSearchInput');
const heroSearchResults = document.getElementById('heroSearchResults');

function heroMatchesQuery(service, query) {
  const q = query.toLowerCase();
  const haystack = [service.name, service.blurb, ...(service.keywords || [])]
    .join(' ')
    .toLowerCase();
...
function renderHeroSuggestions(query) {
  ...
  if (!trimmed || typeof SERVICES_DATA === 'undefined') {
    heroSearchResults.innerHTML = '';
    heroSearchResults.classList.remove('open');
    return;
  }

  const matches = SERVICES_DATA.filter((s) => heroMatchesQuery(s, trimmed)).slice(0, 6);

  if (matches.length === 0) {
    heroSearchResults.innerHTML = `<div class="hero-search-empty">No services found for "${trimmed}". Press Enter to search anyway.</div>`;
    heroSearchResults.classList.add('open');
    return;
  }

  heroSearchResults.innerHTML = matches
    .map(
      (s) => `
      <a href="services.html#${s.id}" class="hero-search-item">
        <span class="hero-search-item-icon">${s.tag}</span>
        <span class="hero-search-item-text">
          <span class="hero-search-item-name">${s.name}</span>
          <span class="hero-search-item-blurb">${s.blurb}</span>
        </span>
      </a>`
    )
    .join('');
...
if (heroSearchForm && heroSearchInput) {
  ...
  heroSearchInput.addEventListener('focus', () => {
    if (heroSearchInput.value.trim()) renderHeroSuggestions(heroSearchInput.value);
  });

  const heroSearchWrap = heroSearchForm.closest('.hero-search-wrap') || heroSearchForm;

  document.addEventListener('click', (event) => {
    if (heroSearchResults && !heroSearchWrap.contains(event.target)) {
      heroSearchResults.classList.remove('open');
    }
  });

  heroSearchInput.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && heroSearchResults) {
      heroSearchResults.classList.remove('open');
    }
  });
